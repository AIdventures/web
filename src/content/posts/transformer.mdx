...
tags:
  ...
  - seq2seq
  - paper-notes
description: 'Demystifying the Transformer architecture, explaining the Encoder, Decoder, and Attention mechanisms block by block with PyTorch implementation'
pubDate: 2017-06-12
...
The <Highlight color="purple">**Encoder**</Highlight> is in charge of transforming the input into a representation
that the decoder can understand. The <Highlight color="brown">**Decoder**</Highlight> is in charge of generating
...
This dictionary is also known as <Highlight color="green">**Vocabulary**</Highlight>.
...
These representations of word mappings are known as <Highlight color="green">**Tokens**</Highlight>.
...
<Highlight color="green">**Word Embeddings**</Highlight>.
...
in the input. This vectors are known as <Highlight color="green">**Positional Embeddings**</Highlight>
...
Well, we <Highlight color="yellow">just add them together</Highlight>. This is the input that we will give to the model.
...
![Transformer input representation. How an input natural lenguage sentence is transformed into a representation that the model can understand.](./images/transformer/transformer_input.png)
...
<Highlight color="green">**Transformer Block**</Highlight>.
...
> the search engine will map your <Highlight color="blue">**query**</Highlight> (text in the search bar)
> against a set of <Highlight color="blue">**keys**</Highlight> (video title, description, etc.)
> associated with candidate videos in their database,
> then present you the best matched videos (<Highlight color="blue">**values**</Highlight>).
...
![Transformer Multi-Head Attention. First step, generate the query, key and value vectors.](./images/transformer/multi-head_first.png)
...
Next the <Highlight color="yellow">**multi-head**</Highlight> part comes into play. The idea is to generate multiple
...
![Transformer Multi-Head Attention. Second step, split the query, key and value vectors into multiple heads.](./images/transformer/multi-head_second.png)
...
With each of the query, key and value vectors generated, we can now generate the <Highlight color="green">**attention matrix**</Highlight>.
This matrix is generated by applying the <Highlight color="yellow">dot product between the query and the key vectors</Highlight>.
The purpose of this is to capture the relationships between the tokens. The higher the dot product,
the higher the relationship between the tokens. The dot product is <Highlight color="yellow">then scaled</Highlight> by the square root
...
![Transformer Multi-Head Attention. Third step, generate the attention matrix.](./images/transformer/multi-head_third.png)
...
This matrix is known as the <Highlight color="yellow">**attention weights**</Highlight>. On top of this matrix
we can apply a Dropout layer to avoid overfitting.

![Transformer Multi-Head Attention. Fourth step, apply the softmax function to the attention matrix.](./images/transformer/multi-head_fourth.png)
...
the <Highlight color="yellow">**context vectors**</Highlight>. This vectors are the ones that will be used
...
![Transformer Multi-Head Attention. Fifth step, apply the attention weights to the value vectors.](./images/transformer/multi-head_fifth.png)
...
![Transformer Multi-Head Attention. Sixth step, combine the context vectors.](./images/transformer/multi-head_sixth.png)
...
![Transformer Multi-Head Attention. Global view of the Multi-Head Attention.](./images/transformer/multi-head_full.png)

### Masked Multi-Head Attention

Also known as <Highlight color="green">**causal** multi-head attention</Highlight>, this part is similar to the multi-head attention
but with a small difference: the attention is masked. This is done because at
[Inference Time](#inference-time)
<Highlight color="yellow">a given *generated* token can only attend to the previous *generated* tokens</Highlight>,
...
![Transformer Masked Multi-Head Attention. The attention matrix is masked before applying the softmax, fourth step in the Multi-Head Attention.](./images/transformer/masked.png)
...
tokens are <Highlight color="red">zeroed out</Highlight>.

### Feed Forward - MLP

The Feed Forward layer is a simple linear transformation followed by a non-linear activation function.
In this case there are <Highlight color="green">**two linear transformations**</Highlight>,
one that <Highlight color="yellow">**expands the dimensionality**</Highlight> of the input by an
arbitrary factor, and another that <Highlight color="yellow">**reduces it back**</Highlight> to the original dimensionality.
...
![Transformer Feed Forward. The input is expanded and reduced back to the original dimensionality.](./images/transformer/mlp.png)
...
<Highlight color="green">**Language Modeling Head**</Highlight>.

![Transformer Feed Forward. The input is expanded and reduced back to the original dimensionality.](./images/transformer/output.png)
...
![Transformer training process. The model is trained in one step.](./images/transformer/training.png)

### Inference Time

At inference time, the model is used to generate the output sequence. This is done
by generating the output <u>token by token</u>, also known as <Highlight color="green">**auto-regressive**</Highlight>.
This is because the output of a given token depends on the previous tokens, so we need
those tokens to generate the next one. This is why we need to <Highlight color="yellow">shift right the output</Highlight>,
so that the first token is the start token `<s>` because we don't have any previous token.

The process would look like:

![Transformer inference process. The model is used to generate the output token by token.](./images/transformer/inference.png)
...
but that is out of the scope of this post. Note how we are using a <Highlight color="yellow">**temperature**</Highlight>
...
If instead we <Highlight color="yellow">**split our embeddings**</Highlight> into multiple $$h$$ heads,
we would end up with <Highlight color="green">**$$h$$ attention matrices**</Highlight>, one per head,
that will allow the model to <Highlight color="yellow">**empower or weaken different parts**</Highlight>
