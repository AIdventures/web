...
which are used to learn a good representation of the data.
In this case, the data are images.
The main goal of DINO is to learn a good representation of the images in an <Underlined color="red">self-supervised way</Underlined>.
...
DINO follows <Underlined color="red">knowledge distillation</Underlined> learning paradigm, where 
...
which is a [Vision Transformer (ViT)](/blog/vit) in this case. A <Underlined>MLP projection head</Underlined> with $K$ dimensions is attached
to compare the outputs of the networks. <Underlined>The features used in downstream tasks come from the ViT backbone</Underlined>.

Given an input image $x$, <Underlined>both networks output probability distributions over $K$ dimensions</Underlined>,
...
To adapt the problem to <Underlined color="red">self-supervised learning</Underlined>, given an image $x$
DINO constructs different distorted views or <Underlined>crops</Underlined> of an image by using a <Underlined color="purple">**multi-crop strategy**</Underlined>.
...
This encourages the <Underlined color="brown">teacher to give us generally a better representation</Underlined> because it receives global views that have more information compared to the local views that the student receives.
At the same time, student learns to recognize fine-grained details and becomes invariant to scale changes.
...
<Underlined>The teacher parameters</Underlined> $\theta_t$ are not given a *priori* and is dynamically built during training.
...
The models are pretrained on the ImageNet dataset without labels.
...
One problem DINO has is that the model can easily collapse,
what leads the model to output the same thing regardless of what the input is.
Why not to output the same embedding for every image and have no error?
...
To avoid this, DINO introduces two strategies:
- **Centering**: Avoids the collapse induced by a dominant dimension. 
    Depends on first-order batch statistics and can be interpreted as adding a bias term $c$
...
Next there is an example of how to visualize the attention maps of the last block of the network.
...
Not resolved yet... Probably trial and error. My theory is that an umbalance is needed so applying the 
same operations not lead us again to collapse.
