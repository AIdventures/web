...
tags:
  ...
  - tutorial
  - receptive-field
description: 'Why do architectures use 3x3 filters? It is because of something called Receptive Fields.'
image: ./images/receptive_fields_cnns/wasteland.jpg
---

Why do architectures use $3\times3$ filters? It is because of something called Receptive Fields.
In this post, we will see what is the receptive field and how it increases over convolutional layers.
Taking this into account, we will see why architectures use $3\times3$ filters.
...
### Single Kernel Case
We have our input image and we pass it through $3\times3$ convolution layers until we get the output.
If we pick an output pixel from the output, that pixel depends on the $3\times3$ receptive field of the previous feature map,
so we can say that the receptive field size of the pixel of the output is 3 so far. 


![Receptive field size increases over multiple convolutional layers](./images/receptive_fields_cnns/one_kernel.png)
...
those provide from a larger $5\times5$ receptive field. If we recap,
till now we have a pixel on the output that it depends on  $3\times3$ receptive field of its previous layer,
which depends on the $5\times5$ receptive field of the second layer. In other words,
if we put two $3\times3$ convolution layers subsequently it has the effect of putting one $5\times5$ convolution layer.

![Receptive field size increases over multiple convolutional layers](./images/receptive_fields_cnns/two_kernel.png)
...
We can see that it covers the $7\times7$ area, which is the whole input.
...
![Receptive field size increases over multiple convolutional layers](./images/receptive_fields_cnns/three_kernel.png)
...
What is the problem with $5\times5$ or $7\times7$ convolutions?
Imagine we have an input volume having $C_1$ channels. 

In the first scenario we use $5\times5$ kernels which of course should have the same number of channels $C_1$ as the input volume and a number of kernels $C_2$. The number of parameters is $(5\times5)C_1 C_2 = 25C_1 C_2$.
...
is lower by using two $3\times3$ kernels** subsequently.
A final advantage of using two $3\times3$ kernels subsequently instead a $5\times5$ kernel
is that when using $3\times3$ kernels when introduce in between activation maps
which adds **more non-linearity** to our model.


## The Pooling layers

Imagine we have an input size of $224\times224$ and our kernel size is $3$.
Based on the formula that receptive field is
$1 + L(k-1)$ we can write $1+L(3-1) =224 \rightarrow L \approx 112$.
That basically means that we need to add $112$ $3\times3$ convolution layers to our neural network
...
If we apply a Max Pooling with kernel size $2\times2$ and stride 2,
...
![Receptive field size increases over multiple convolutional layers](./images/receptive_fields_cnns/max_pool.png)
...
- <a href="/diagrams/receptive_fields_cnns.excalidraw" download="receptive_fields_cnns.excalidraw">Receptive Fields CNN Excalidraw diagram source</a>