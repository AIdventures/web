...
tags:
  ...
  - paper-notes
  - senet
description: 'Introducing channel attention to improve the performance of image classification tasks'
pubDate: 2017-09-17
...
called <Highlight color="green">channel attention</Highlight> which learns to emphasize
informative features and suppress less useful ones selectively.
This is achieved by using a <Highlight color="yellow">squeeze operation</Highlight> to compress the spatial dimensions
of feature maps into a vector, followed by an <Highlight color="yellow">excitation operation</Highlight> that learns to
...
<Highlight color="yellow">channel attention helps to filter out noise and irrelevant features</Highlight>,
...
<Highlight color="yellow">The features U are first passed through a squeeze operation, which produces a channel descriptor
by aggregating feature maps across their spatial dimensions (H × W)</Highlight>.
...
<Highlight color="yellow">The aggregation is followed by an excitation operation,
which takes the form of a simple self-gating mechanism that takes the embedding as input
and produces a collection of per-channel modulation weights.</Highlight>
...
![Squeeze-and-Excite residual module](./images/senet/squeeze_excite.png)
...
by explicitly <Highlight color="yellow">modeling channel interdependencies</Highlight> so that the network
...
<Highlight color="green">squeeze global spatial information into a channel descriptor</Highlight>.
This is achieved by using <Highlight color="yellow">global average pooling</Highlight> to generate channel-wise statistics.
...
We opt to employ a <Highlight color="yellow">simple gating mechanism with sigmoid activation</Highlight>.
To limit <Highlight color="green">model complexity and aid generalization</Highlight>,
we parameterize the gating mechanism by forming a <Highlight color="green">bottleneck</Highlight> 
with two fully-connected (FC) layers around the non-linearity, i.e.,
a dimensionality-reduction layer with <Highlight color="yellow">reduction ratio $r$</Highlight>, a ReLU and then a dimensionality-increasing layer
...
Here it is the standard SE block, which uses a <Highlight color="yellow">residual connection</Highlight>,
...
An analogous to [ResNet18](/posts/resnet) can be instantiated as follows:
...
- <a href="/diagrams/senet.excalidraw" download="senet.excalidraw">SENet Excalidraw diagram source</a>
