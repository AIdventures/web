...
Previous methods problems (transformer) => unidirectionality constraint.
Unlike left-to-right language model pre-training, the masked language model (MLM)
objective enables the representation to fuse the left and the right context.
we also use a “next sentence prediction” task that jointly pretrains text-pair representations

There are two steps in our framework: pre-training and ﬁne-tuning.
During pre-training, the model is trained on unlabeled data over different pre-training tasks.
For ﬁnetuning, the BERT model is ﬁrst initialized with the pre-trained parameters,
and all of the parameters are ﬁne-tuned using labeled data from the downstream tasks.
A distinctive feature of BERT is its uniﬁed architecture across different tasks.

Transformer encoder based on the original implementation


One sentence or multiple => Separator [SEP] and sentence embeddings
+ classiﬁcation token [CLS]
