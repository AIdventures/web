...
BERT paper proved that Transformer (self-attention) based encoder can be powerfully used
...
<Underlined color="brown">**Transformer Encoder**</Underlined>,
...
ny [Transformer blog post](/blog/transformer).
...
Both tasks are trained at the same time, summing their losses. For the pre-training
...
At the output, the token representations are fed into an output layer for token-level
tasks such as named entity recognition, and the `[CLS]` representation is fed into
