...
Self-attention-based architectures, in particular [Transformers](./transformer), have become the model of choice in natural language processing (NLP). The predominant approach is to pre-train on a large text corpus and then fine-tune on a smaller task-specific dataset.
...
ViT's model architecture is a multi-layer <Highlight color="brown">bidirectional Transformer Encoder</Highlight>, inspired in [BERT](./bert) and following the original [Transformer](./transformer) design as closely as possible.
